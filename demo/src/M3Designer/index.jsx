...
const M3Designer = ({ fieldsSchema, onOK, onCancle, designItem }, ref) => {
  ...
  const changeCurSchema = (index) => {
    ...
    setIdx(i);
    const lSchema =
      i >= 0 && schema?.objectFields?.length > 0 ? schema.objectFields[i] : {};
  ...
  // 添加选项
  const addInput = (obj) => {
    const currentSchema = form.getSchema();
    const opt = { name: `t_${uuid()}`, relevance: {}, ...obj };

    // 创建新的 objectFields 数组，避免直接修改原数组
    const newObjectFields = [...currentSchema.objectFields];

    if (idx == -1) {
      newObjectFields.push(opt);
      changeCurSchema(newObjectFields.length - 1);
      scrollToBottom("m3-designer-wrap");
    } else {
      newObjectFields.splice(idx + 1, 0, opt);
      changeCurSchema(idx + 1);
    }

    // 只更新 form 的 schema，不触发 setSchema
    const newSchema = {
      ...currentSchema,
  ...
  const lSchema =
    idx >= 0 && schema?.objectFields?.length > 0
      ? schema.objectFields[idx]
      : {};
  return (
    <div id="m3-designer-wrap" className={"m3-designer-wrap"}>
      {/* 左侧题目的设计器 */}
      <div className={"left-operation"}>
        {schemaleft?.type ? (
          <>
            {schemaleft?.editor == "NPS" ? (
              schemaleft?.label2 == "课程评价NPS" ? (
                <div className={"top"}>
                  <Tooltip title={"课程评价NPS，该数据会计入课评NPS报表中"}>
                    课程评价NPS
                  </Tooltip>
                </div>
              ) : schemaleft?.label2 == "老师评价NPS" ? (
                <div className={"top"}>
            ...
            ) : null}
            {
              <>
                ...
                <MViewerDebug
                  ...
                  afterChange={function (path, v, a, cb) {
                    if (path === "max" || path === "min") {
                      if (
                        _.isNumber(schemaleft.max) &&
                        _.isNumber(schemaleft.min) &&
                        schemaleft.max < schemaleft.min
                      ) {
                    ...
                    // 直接更新 schema.objectFields[idx]，不触发整个组件重新渲染
                    if (idx >= 0) {
                      const currentSchema = form.getSchema();
                      const newObjectFields = [...currentSchema.objectFields];
                      console.log("newObjectFields", {
                        currentSchema,
                        newObjectFields,
                        idx,
                    ...
                    <div>
                      <Button
                        disabled={
                          ...
                          schemaleft?.option.length > 0 &&
                          schemaleft?.option.every((item) => item.label)
                            ? false
                            : true
                        }
                        type={
                          JSON.stringify(schemaleft?.relevance).match(
                            /\[([^\]]+)\]/
                          )
                            ? "primary"
                            : "default"
                ...
                {schemaleft?.errorTip ? (
                  <div className={"error-tip"}>{schemaleft.errorTip}</div>
                ) : null}
              </>
            }
          </>
        ) : (
          <div className={"none-box"}>点击中间区域的控件设置字段</div>
        )}
      </div>
      ...
      <div id="middle-operation" className={`middle-operation ql-editor`}>
        {schema?.objectFields?.length > 0 ? (
          <div className={MUtil.phoneLike() ? `MEditor_p` : `MEditor`}>
            <M3
              ...
              formItemWrapper={(ele, _schema) => {
                const curIdx = schema.objectFields.findIndex(
                  (i) => i.name === _schema.name
                );
                return (
                  <div
                    key={_schema.name}
                    ...
                    className={
                      idx === curIdx
                        ? `${"m3-designer-fliedbox"} ${"m3-designer-fliedbox-active"}`
                        : `${"m3-designer-fliedbox"}`
                    }
                  >
                    {schema.objectFields[curIdx]?.errorTip ? (
                    ...
                    <div className={"operate-btn-box"}>
                      <span className={"id-text"}>{_schema.name}</span>
                      {curIdx !== 0 ? (
                        <div
                          ...
                          onClick={(e) => {
                            ...
                            const newObjectFields = [
                              ...currentSchema.objectFields,
                            ];
                            const temp = newObjectFields[curIdx];
                            newObjectFields[curIdx] =
                              newObjectFields[curIdx - 1];
                            newObjectFields[curIdx - 1] = temp;
                      ...
                      {curIdx !== schema.objectFields.length - 1 ? (
                        <div
                          className={"operate-btn"}
                          onClick={(e) => {
                            const currentSchema = form.getSchema();
                      ...
                      <div
                        className={
                          _schema.canDelete == false
                            ? `${"operate-btn"} disable-link-text`
                            : "operate-btn"
                        }
                      >
                        <Popconfirm
                          ...
                          onConfirm={() => {
                            ...
                            newObjectFields.splice(curIdx, 1);
                      ...
                      <div
                        ...
                        onClick={(e) => {
                          ...
                          newObjectFields.splice(curIdx + 1, 0, copySchema);
          ...
          <div className={"none-box"}>点击右侧控件添加字段</div>
        )}
      </div>
                ...
                {item?.tooltips ? (
                  <Tooltip title={item.tooltips}>
                    <QuestionCircleOutlined
                      style={{ color: "#1890ff", paddingLeft: "5px" }}
                    />
                  </Tooltip>
                ) : null}
              </div>
        ...
        <div className={"m3-designer-relevance"}>
          <div className={"select-value"}>
            ...
            <div className={"select-wrap"}>
              {
                // @ts-ignore
                _.isArray(schemaleft?.option)
                  ? schemaleft.option.map((item, index) => {
                      const len = schemaleft.relevance
                        ? schemaleft.relevance[item.value]
                          ? schemaleft.relevance[item.value].length
                          : 0
                        : 0;
                      return (
                        <div
          ...
          <div className={"middle-icon"}>
            ...
            >
              <path
                d="M502.88579318 562.12517926h265.32856415a124.27567408 124.27567408 0 1 0-16.777216-62.13783704h-248.55134815V143.937536h270.29959112a114.95499852 114.95499852 0 1 0 111.84810666-141.05289008 114.33362015 114.33362015 0 0 0-101.90605274 62.13783705H471.1954963a36.03994548 36.03994548 0 0 0-31.06891852 39.7682157v395.19664355H234.45033718a116.19775525 116.19775525 0 1 0-12.4275674 62.13783704h218.72518637v333.05880652a36.03994548 36.03994548 0 0 0 31.06891851 39.76821571h294.53334756a124.27567408 124.27567408 0 0 0 109.36259319 62.13783703 116.81913363 116.81913363 0 1 0 0-233.01688888 124.27567408 124.27567408 0 0 0-124.27567408 91.96399881h-248.55134815z m372.82702223-108.74121481a42.25372918 42.25372918 0 0 1 36.03994547 19.26272947 36.66132385 36.66132385 0 0 1 0 38.52545897 42.25372918 42.25372918 0 0 1-36.03994547 19.26272948 39.14683733 39.14683733 0 1 1 0-77.67229629z m9.32067555-372.82702223A39.14683733 39.14683733 0 1 1 844.02251851 117.83964445a38.52545897 38.52545897 0 0 1 41.01097245-37.28270223zM153.67114903 526.70661215a37.90408059 37.90408059 0 0 1-32.93305362 19.26272949 38.52545897 38.52545897 0 0 1 0-77.6722963 37.90408059 37.90408059 0 0 1 32.93305362 19.26272948 39.14683733 39.14683733 0 0 1 0 39.14683733z m722.04166638 314.41745541a39.14683733 39.14683733 0 1 1-41.63235083 38.52545896 40.38959408 40.38959408 0 0 1 41.63235083-38.52545896z"
                fill="#333333"
                p-id="3869"
              ></path>
            </svg>
            ...
            <div className={"select-wrap"}>
              <Checkbox.Group
                value={
                  (schemaleft &&
                    schemaleft?.option &&
                    schemaleft.option[optionIdx] &&
                    schemaleft.option[optionIdx].value &&
                    schemaleft.relevance &&
                    schemaleft.relevance[
                      schemaleft?.option[optionIdx].value
                    ]) ??
                  []
                ...
                onChange={(checkedValues) => {
                  if (schemaleft.relevance) {
                    schemaleft.relevance[schemaleft?.option[optionIdx].value] =
                      checkedValues;
                  }
                }}
              >
                {(
                  (schema &&
                    schema.objectFields &&
                    schema.objectFields.slice(idx + 1)) ||
                  []
                ).map((item) => {
                  return (
                    <div style={{ padding: "0 5px" }} key={item.value}>
