...
export const unitEnum = [
    ...
    { type: "int", label2: "课程评价NPS", npsType: 1, editor: "NPS", icon: 'nps', tooltips: "课程评价NPS，该数据会计入课评NPS报表中" },
    { type: "int", label2: "老师评价NPS", npsType: 2, editor: "NPS", icon: 'nps', tooltips: "老师评价NPS，该数据会计入老师NPS报表中" },
    {
        type: "decoration",
        label2: "分段标题",
        icon: 'segmentLabel',
    ...
    {
        type: "attachment", label: "附件上传", props: {
            maxSize: 100,
            maxAmount: 1
        }
    },
...
export const generateItemSchema = (curSchema) => {
    ...
    if (type === 'decoration') {
        ...
        base = [
            { label: "标题", name: "label", type: "string", defaultValue: "我对本次课程的收获感打分是？" },
            { label: "是否必填", layoutHint: 'h', name: "required", type: "enum", editor: "ARadio", enumFields: [{ label: "是", value: true }, { label: "否", value: false }] },
            {
                label: "描述文案", name: `props`, editor: NpsProps, defaultValue: {
                    leftTip: '没有收获',
                    rightTip: '极有收获',
        ...
        base = [
            { label: "标题", name: "label", type: "string", defaultValue: "我对老师的评价？" },
            {
                label: "请选择老师", name: "archive", type: "vl", editor: "ARemoteSelector",  placeholder: "请输入",
                remote: {
                ...
                props: {
                    ...
                    onChange: function (v) {
                        ...
                        let c = ''
                        if (v && v.label) {
                            c = db.label.replace(/对(.*?)老师/,`对${v.label.split('(')[0]}老师`);
                        } else {
        ...
        base = [
            ...
            { label: "是否必填", layoutHint: 'h', name: "required", type: "enum", editor: "ARadio", enumFields: [{ label: "是", value: true }, { label: "否", value: false }] },
            editor === 'ARate' ? { label: "星数", name: "max", type: "int", min: 1, max: 100, defaultValue: 5 } : null,
            editor === 'ARate' ? { label: "描述文案", name: `props`, editor: RateProps } : null,
            editor === 'NPS' ? { label: "描述文案", name: `props`, editor: NpsProps } : null,
            type === 'string' ? { label: "输入提示", name: "placeholder", type: "string", defaultValue: '请填写', placeholder: "请填写输入提示" } : null,
            type === 'set' || type === 'enum' ? { label: "选项", name: `option`, type: "array", editor: "AArrayGrid", arrayMember: valueLabel, autoValue: true } : null,
            type === 'set' || type === 'enum' ? { label: "其他选项", name: `openOption`, editor: OpenOption } : null,
            (editor !== 'ARate' && editor !== 'NPS') && (type === 'int' || type === 'string' || type === 'set') ? { label: type === 'set' ? '至少选择几项' : (type === 'int' ? "可填写的最小值" : "最少填写多少字"), name: "min", type: "int", max: 100000, placeholder: "请填写数字" } : null,
            (editor !== 'ARate' && editor !== 'NPS') && (type === 'int' || type === 'string' || type === 'set') ? { label: type === 'set' ? '至多选择几项' : (type === 'int' ? "可填写的最大值" : "最多填写多少字"), name: "max", type: "int", max: 100000, placeholder: "请填写数字" } : null,
            type === 'string' ? { label: "最多展示几行的输入，默认为1", name: "stringLines", type: "int", min: 1, max: 100, placeholder: "请填写数字" } : null,
        ]
        if (type === 'attachment') {
            base.push(
                { label: "其他配置", name: `props`, editor: OssuploadProps }
            )
        }
    }
